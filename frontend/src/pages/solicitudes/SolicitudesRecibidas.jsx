...
import { listarSolicitudesRecibidas } from '../../services/solicitudService';
...
export default function SolicitudesRecibidas() {
  ...
  useEffect(() => {
    ...
    listarSolicitudesRecibidas(token)
  ...
  return (
    <div style={styles.pagina}>
      <div style={styles.contenedor}>
        <h1 style={styles.titulo}>Solicitudes recibidas</h1>
        {error && <p style={styles.errorMsg}>{error}</p>}
        {cargando && <p>Cargando...</p>}
        {!cargando && solicitudes.length === 0 && <p style={styles.vacio}>No has recibido solicitudes aún.</p>}
        {solicitudes.map((s) => {
            ...
            <Link key={s.id} to={`/solicitudes/${s.id}`} style={styles.enlace}>
              <div style={styles.fila}>
                <div style={styles.filaInfo}>
                  <span style={styles.org}>{s.organizador.usuario.nombre}</span>
...
const styles = {
  ...
  org: { fontWeight: 600, color: 'var(--color-texto)', fontSize: '1rem' },
